...
function MovementForm({ onClose }: { onClose: () => void }) {
  ...
  const { data: products = [] } = useQuery({
    queryKey: ['products-mov', productSearch],
    queryFn: async () => {
      ...
      <DialogContent>
        ...
        <form
          onSubmit={handleSubmit((d) => {
            ...
            mutation.mutate({ ...d, productId: selectedProduct.id, type: movType })
          })}
        >
              ...
              {selectedProduct ? (
                <div className="flex items-center justify-between p-3 rounded-lg bg-secondary border border-border">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Stock: {selectedProduct.currentStock}</p>
                  </div>
                  ...
                  {products.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-popover border border-border rounded-xl shadow-xl z-20 max-h-40 overflow-y-auto">
                      {products.map((p: any) => (
                        ...
                        >
                          <span>{p.name}</span>
                          <span className="text-xs font-mono text-muted-foreground">Stock: {p.currentStock}</span>
                        </button>
                      ))}
                    </div>
                  )}
...
export default function Stocks() {
  ...
  const { data: inventory, isLoading: invLoading } = useQuery({
    ...
    queryFn: async () => {
      const params: any = { page: invPage, limit: 10 }
  ...
  const { data: movements, isLoading: movLoading } = useQuery({
    ...
    queryFn: async () => {
      const r = await api.get('/stocks/movements', { params: { page, limit: 10 } })
  ...
  const { data: alerts = [] } = useQuery({
    ...
    queryFn: async () => {
      const r = await api.get('/stocks/alerts')
      return extractList<any>(r)
    },
  })
  ...
  const tabs = [
    ...
    { id: 'alerts', label: `Alertes ${alertItems.length > 0 ? `(${alertItems.length})` : ''}` },
  ]
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Gestion des entrées, sorties et inventaire</p>
        </div>
      ...
      <Card>
          ...
          invLoading ? (
            ...
            <div>
              <div className="flex items-center gap-3 px-5 py-3 border-b border-border">
                ...
                <Select onValueChange={(v: string) => { setInvCategory(v === '__all__' ? undefined : v); setInvPage(1) }} value={invCategory || '__all__'}>
                ...
                <Select onValueChange={(v: string) => { setInvBrand(v === '__all__' ? undefined : v); setInvPage(1) }} value={invBrand || '__all__'}>
                  <SelectTrigger className="min-w-[160px]"><SelectValue /></SelectTrigger>
                  <SelectContent>
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => { setInvLowStock((v) => !v); setInvPage(1) }}
                >
                  {invLowStock ? 'Filtre: Faible' : 'Afficher Faible'}
                </Button>
              </div>

              {invProducts.length === 0 ? (
                <EmptyState icon={<Warehouse className="h-6 w-6" />} title="Inventaire vide" />
              ) : (
                <>
                  <Table>
                    <TableHeader>
                  ...
                  <div className="px-5 py-3 border-t border-border flex items-center justify-end">
                    <Pagination page={invPage} totalPages={invMeta.pages || 1} total={invMeta.total || 0} limit={10} onPageChange={setInvPage} />
                  </div>
                </>
              )}
            </div>
        ...
        {activeTab === 'movements' && (
          movLoading ? (
            ...
            <EmptyState icon={<ArrowUpDown className="h-6 w-6" />} title="Aucun mouvement" />
          ) : (
            <>
              <Table>
                <TableHeader>
              ...
              <Pagination page={page} totalPages={movMeta.pages || 1} total={movMeta.total || 0} limit={10} onPageChange={setPage} />
            </>
          )
        )}
        ...
        {activeTab === 'alerts' && (
          alertItems.length === 0 ? (
            <EmptyState icon={<AlertTriangle className="h-6 w-6" />} title="Aucune alerte" description="Tous vos stocks sont à niveau." />
          ) : (
